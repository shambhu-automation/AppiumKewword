...
public class TestController extends TestBase{
	...
	@Test
	public void TestCaseController() throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, IOException, InterruptedException {
		
		String startTime = TestBase.now("dd.MMMM.yyyy hh.mm.ss aaa");
		ReportUtil.startTesting("index", startTime, "Test", "1.0");
		...
		for(int TC=2;TC<=SuiteData.getRowCount("TestCases");TC++) {
			...
			if(RunMode.equals("Y")) {
				...
				driver = (AppiumDriver) TestBase.OpenApplication(Repository.getProperty("platformName"));
